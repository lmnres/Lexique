...
text_input = "le cinéma a changé depuis que les actrices ont montré leurs convictions sur les droits de la femme et l'égalité homme femme. ceux que nous connaissons sont aussi pour les droits de l'homme"
text_output = []
...
for word in text_input.split():
    if lex['ortho'].str.contains(word).any():
        ...
        if (data_row.iloc[0]["genre"] == "m") and (data_row.iloc[0]["nombre"] == "p") and (data_row.iloc[0]["cgram"]=="NOM"):
            ...
            try :
                if word < complementaire.iloc[0]["ortho"] :
                    text_output.append(word)
                    text_output.append("et " + complementaire.iloc[0]["ortho"])
                else :
                    text_output.append(complementaire.iloc[0]["ortho"])
                    text_output.append("et " + word)
            except:
                pass
